      ...
      },
      {
        ...
        stages: [
          ...
          },
          {
            ...
            detail:
              'market-stats:All is the one payload that cannot be summed — a median needs the whole distribution, and means need their own denominators. One query returns every pool as a count plus its percentile_cont(0.5) / avg, and marketStatsFromPools() formats those into the same payload shape the per-town path produces. Per-town market-stats also stores medianPrice12Mo (trailing 12 months of closed prices) for Deal of the Day; Pulse still shows medianPrice (2024–YTD closed). Reading every Active and Closed listing in the market to reduce them in Node is what exhausted V8\'s heap, and that cost grew with each town added. Tax (market-pulse-tax:*:v4) is the same shape: SQL percentile_cont / avg over every listing for one fiscal year (current once 80% of the book has it, otherwise prior), after CAMA has run, excluding Matrix all-nines placeholder bills, not Active-only, and not a client rollup.',
            status: 'live',
            statusLabel: 'Live',
          },
          {
            ...
            detail:
              'Each stats rebuild freezes archived Mondays from market_pulse_snapshots onto market-pulse-week:sale:all:v1:{slotDate}. Live stacked numbers only fill the current slot when that week has not been archived yet. A missing prior Monday is reconstructed from listings as-of that send-day so Week Over Week does not wait for a second real send. clearStatsCache preserves the prefix so the timeline can walk backwards. The /market-pulse switch is Off / Week Over Week; listing pulse and home pulse stay Off. The Monday email always draws the week-over-week change in the gold fill.',
            status: 'live',
            statusLabel: 'Live',
          },
          {
