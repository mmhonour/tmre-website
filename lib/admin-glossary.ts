...
export const ADMIN_GLOSSARY: GlossaryEntry[] = [
  ...
  },
  {
    ...
    category: 'sync-admin',
    definition:
      'Scheduled VGSI GIS crawler (Westport first): Streets.aspx → Parcel.aspx Field Card parse → Neon `vision_addresses` typed columns + `field_card` jsonb (labeled pairs + searchText for Find) + optional R2 HTML pointer for reference. Each chunk first fills any missing `vision_streets` letters (A–Y except X/Z) without moving the parcel cursor, then walks parcels. After a town’s street alphabet completes, phase flips to incremental re-crawl comparing `content_fingerprint` (VGSI has no known modified-since feed). Default chunk is 40 parcels (Admin/Netlify, hard cap 200). CLI loops 1000-parcel chunks until the town is complete (`VISION_SYNC_TARGET=neon`); `VISION_SYNC_ONCE=1` for a single chunk. While running, each parcel logs to the console and stamps `vision_addresses_live` (Admin Status shows current address). `scraped_at` is ISO-8601 UTC (Postgres `timestamptz` `+00`). Each successful chunk ends with Vision listing match (same stack as prod). Admin Sync now queues the background worker (same as the thin cron) so the Admin POST does not 504. CLI `npm run sync:vision-addresses`. Distinct from property-addresses (List With Me thin directory). Homepage: Westport Vision GIS homepage.',
  },
  {
    term: 'Brokerage name',
    category: 'sync-admin',
