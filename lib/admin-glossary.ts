...
export const ADMIN_GLOSSARY: GlossaryEntry[] = [
  ...
  },
  {
    ...
    category: 'sync-admin',
    definition:
      '“Modified since” RETS pull using ModificationTimestamp — Active/CS/UC plus recently modified Closed (upserted into the Closed bucket so just-sold comps appear without a full Closed sync). Netlify sync-listings (every 30m, not background) always runs a lean RETS-only pull in-process, stamps last_incremental_cron_tick, and optionally queues sync-listings-worker for board/stats warm + spotlight/alerts. Inventory freshness must never depend on that hop. Weekly/monthly jobs use the same thin-schedule → *-worker pattern. Combining schedule+background on one Netlify function is a silent no-op (Day-1 failure mode). Admin Syncs shows the heartbeat; Database Next is only clock-slot math when overdue.',
  },
  {
