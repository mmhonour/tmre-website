...
export const ADMIN_GLOSSARY: GlossaryEntry[] = [
  ...
  },
  {
    ...
    category: 'sync-admin',
    definition:
      'Railway queue job (`hero-photos`) that scavenges leftover Active listings missing six showcase hero index slots (`?size=full`) in R2. Oldest `list_date` first, five listings per hop, hops walk on inside the ~9-minute burst. A hop that stores nothing is skipped next hop and next burst (capped at 400, then wrap and retry). First pass does not abort on empties — unfillable oldest must not block fillable leftovers. After wrap, three empty hops with zero fills abort the burst (Media/RETS down) but still persist skip. Full-size fetch is MediaURL first, then RETS Photo/LargePhoto/HiRes (never Thumbnail). R2 bytes with a missing index row are indexed without a Media refetch. Lowest claim rank so Incremental / stats / CAMA go first. Writes `hero_photos_status` (percent missing before against the before-burst Active count, listings/photos filled, percent remaining) and `last_hero_photos`. When coverage is 100% the job still runs, only recounts, and reports idle / 100% complete on the Syncs run board. Not part of Sync all. Railway sweeps every 2m and thin cron `sync-hero-photos` every 15m; both still respect Configure. Optional daily CLI `npm run backfill:listing-photos` does full galleries (and other statuses) when an operator has time; this job picks up first-six Active slack in between. Incremental still queues ids only — it does not download photo bodies.',
  },
  {
