...
export const ADMIN_GLOSSARY: GlossaryEntry[] = [
  ...
  },
  {
    ...
    category: 'sync-admin',
    definition:
      'Admin row label for the Railway queue job (`hero-photos`, thin cron `sync-hero-photos`). Scavenges leftover photos for every listing still missing R2/index slots — Active, Closed, Expired, every photo index up to 60, not only the showcase six. Active leftovers first, then other statuses, harvest order from Configure (default newest `list_date`; also oldest, oldest Closed, almost complete). Five listings per hop, hops walk on inside the ~9-minute burst and stop mid-listing when the clock runs out so a 40-photo Closed row cannot overrun the child. A hop that stores nothing is skipped next hop and next burst (capped at 400, then wrap and retry). First pass does not abort on empties. After wrap, three empty hops with zero fills abort the burst (Media/RETS down) but still persist skip. Full-size fetch is MediaURL first, then RETS Photo/LargePhoto/HiRes (never Thumbnail). R2 bytes with a missing index row are indexed without a Media refetch. Lowest claim rank so Incremental / stats / CAMA go first. Writes `hero_photos_status` (Started / Filled / End lines: leftover listings that still have holes, photos pushed this burst with avg per listing, and how many of those listings are now fully in R2). Leftover count only falls when a listing’s last missing slot is stored — a 9-minute burst that adds 24 photos to a 40-slot Closed gallery still counts as missing. `last_hero_photos` is the End stamp. When coverage is 100% the job still runs, only recounts, and reports idle / 100% complete on the Syncs run board. Not part of Sync all. Railway sweeps every 2m and thin cron `sync-hero-photos` every 15m; both still respect Configure. Optional daily CLI `npm run backfill:listing-photos` is faster catch-up when an operator has time; this job picks up remaining slack in between. Incremental still queues ids only — it does not download photo bodies. Showcase “hero six” is still Site warm for new Incremental inserts — see Hero (photo).',
  },
  {
  ...
  },
  {
    ...
    category: 'sync-admin',
    definition:
      'Scheduled VGSI GIS crawler (Westport first): Streets.aspx → Parcel.aspx Field Card parse → Neon `vision_addresses` typed columns + `field_card` jsonb (labeled pairs + searchText for Find) + optional R2 HTML pointer for reference. Each chunk first fills missing `vision_streets` letters and `vision_street_parcels` house lists for every town in `VISION_GIS_TOWNS` (add a town there — no Sync now). Then it ingests Field Cards for street-house PIDs that still lack `owner_name` (fillMissingStreetParcelOwners — same Parcel.aspx path, does not move the letter cursor). Catch-up off the weekly slot (`visionGisNeedsCatchUp`) is only incomplete letters/houses or street-house PIDs with no `vision_addresses` row. A stored Field Card that has owner_name but no mailing / Owner address is not catch-up — that used to re-queue the same 40 PIDs every 10 minutes forever. Weekly fill may retry blank owner_name after 7 days. Remaining budget walks Field Cards for the current crawl town. After a town’s street alphabet completes, phase flips to incremental re-crawl comparing `content_fingerprint` (VGSI has no known modified-since feed). Default chunk is 40 parcels (hard cap 200). CLI loops 1000-parcel chunks until the town is complete (`VISION_SYNC_TARGET=neon`); `VISION_SYNC_ONCE=1` for a single chunk; letter-index only is `npm run sync:vision-streets`. While running, each parcel logs to the console and stamps `vision_addresses_live` (Admin Status shows current address). `scraped_at` is ISO-8601 UTC (Postgres `timestamptz` `+00`). Each successful chunk ends with Vision listing match (same stack as prod). Admin Sync now and the thin cron enqueue on `sync_queue`; the Railway runner claims and forks (Netlify `sync-vision-addresses-worker` only if the row is stranded). History End is the chunk finish, not the start (`syncedAt`). Distinct from property-addresses (List With Me thin directory). Homepage: Westport Vision GIS homepage.',
  },
  {
    term: 'Brokerage name',
    category: 'sync-admin',
  ...
  },
  {
    ...
    category: 'product',
    definition:
      'Market Pulse town sort. Default is Seller Friendly. STACKED uses a composite (months supply + avg DOM). UNSTACKED sorts each chart on its own metric — Seller: DOM ascending, closed sales descending, median/average price descending; Buyer is the reverse. The Town metrics heading shows STACKED | UNSTACKED only after All towns is expanded; collapsing All towns forces stacked so unstacked cannot repeat All-towns-only charts. All towns stays on top and is the only town row until it is clicked. The panel Sort ▲▼ toggles Seller / Buyer Friendly. Median, Delta, and Average are sandwiched (no gap); Delta is mean minus median in $K and as % of median, drawn as the span between the Median and Average bar ends on the same dollar scale. Coming soon (footer on /market-pulse): Active Listings ÷ Housing Units and 24-Month Closings ÷ Housing Units. Scoring in lib/market-pulse-favorability.ts.',
  },
  {
  ...
  },
  {
    ...
    category: 'sync-admin',
    definition:
      '“Modified since” RETS pull using ModificationTimestamp — Active/CS/UC plus recently modified Closed and Expired (upserted into those buckets). Expired uses the same StatusChangeTimestamp window as Closed because MLSStatus=|X alone returns no rows. An empty Expired bucket also triggers a one-per-day year-window catch-up so /new-construction/expired-listings can fill after full-resync was retired. Netlify sync-listings (every 30m, not background) always runs a lean RETS-only pull in-process, stamps last_incremental_cron_tick, and optionally queues sync-listings-worker for board/stats warm + spotlight/alerts. Inventory freshness must never depend on that hop. Admin Sync now runs the same RETS path but does not stamp the cron tick — “Cron last fired: never” with a recent Start/End usually means a manual sync, not a missed */30. Weekly/monthly jobs use the same thin-schedule → *-worker pattern. Combining schedule+background on one Netlify function is a silent no-op (Day-1 failure mode). Admin Syncs shows the heartbeat (Fresh from Postgres); Database Next when overdue is clock-slot math (:00/:30), not proof cron ran.',
  },
  {
  ...
  },
  {
    ...
    category: 'sync-admin',
    definition:
      'Neon table of public SmartMLS OpenHouse events (db/migrations/0023_open_houses.sql). /open-houses reads this table only — no RETS on the page. The hourly queue job replaces today through today+6 (seven calendar days) so a cancelled showing disappears, stamps open_houses_synced_at, then upserts lookback in newest-first 14-day slices under an 8-minute budget so a long history pull cannot hide a finished upcoming write. Rows older than the lookback horizon or after the t+6 inventory are pruned. After a successful pull, that seven-day JSON is written to stats_cache (`open-houses:remaining-week`). /open-houses embeds that JSON in the HTML (cache peek only) and filters it to the page window: Sunday 12:00 AM ET resets to Sunday–Saturday; Monday is Monday–Sunday; Tuesday–Saturday is the remaining days through Sunday. A miss falls back to `/api/listings/open-houses`. Past / upcoming counts document earlier and later showings on those same homes, not series that already ended. The listing History panel lists stored OpenHouse rows for this MLS id and prior listings at the address. Show by is All, Most open houses (top 3 historical hosts per town, ties stay), or First showing (0 past). Sale / rental and town pills sit in the navy hero and dock into the sticky cream bar on scroll. Date / By day / Price sit left on the glyph row; Open house alerts is centered; Show by and Large / Grid / Line sit right. MLS times stay text, not timestamps, so 11am stays 11am in Connecticut. Town sections start collapsed; drag the town name to reorder.',
  },
  {
    term: 'sync_queue',
    category: 'sync-admin',
  ...
  },
  {
    ...
    category: 'sync-admin',
    definition:
      'The loop inside the always-on Railway mls-sync service (services/mls-sync/job-runner.ts) that claims waiting sync_queue rows with SELECT … FOR UPDATE SKIP LOCKED, forks a child per row, heartbeats while it runs, and writes the outcome back. Up to MLS_SYNC_MAX_CHILDREN (default 3) different jobs run at once — Incremental can pull while stats rebuilds and CAMA fills tax history. The same job_id still cannot run twice (unique index). It also reaps rows whose running process stopped heartbeating. A budget kill or a real child crash cools that job down for 30 minutes so it cannot spin; a host-loss reap (deploy / dead parent) does not. Its heartbeat is what Netlify checks before deciding a queued row is stranded.',
  },
  {
  ...
  },
  {
    ...
    category: 'sync-admin',
    definition:
      'Neon table of weekly Market Pulse / Monday brief payloads. One row per Eastern send-day (slot_date). Written after a real send and by `npm run snapshot:market-pulse`. Send test does not insert a row. Stacked town points for week / month / year compares also land in stats_cache as preserved `market-pulse-week:sale:all:v1:{slotDate}` keys (snapshots freeze that Monday; a missing prior Monday is reconstructed from listings as-of that send-day). /market-pulse subtracts a prior week slot from the live stacked defaults (server-side); the page switch is Off / Week Over Week (month stays off the switch). Listing town pulse and home pulse stay Off. The Monday email always draws the week-over-week change in the gold fill.',
  },
  {
