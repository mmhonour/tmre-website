...
export const ADMIN_GLOSSARY: GlossaryEntry[] = [
  ...
  },
  {
    ...
    category: 'product',
    definition:
      'When /find typeahead can only match a narrow searchable set — historically MLS rows in the listings table — so suggestions feel sparse even if the API is fast. /find is Westport Lookup: typeahead is vision_addresses (cadastral) plus Westport MLS streets that GIS has not ingested yet. Matches street address, owner_name, owner_mailing_address, MBLU, account number, Vision PID, Field Card searchText, and MLS#. Off-market parcels open /find/westport/{vision_pid} with the Vision field card on the page; on-market rows merge listing-wins + Vision gap-fill. If the parcel is not in listings, a one-off RETS pull (Vision MLS id/key, else one address search) upserts it permanently via persistListingByMlsId / persistListingRecord and stamps listings.vision_pid + vision_addresses.listing_id. A “Listing is available” banner shows at the top of the parcel page only on that ingest request. Typing an MLS# that is missing from listings does the same persist. Incomplete GIS fill no longer hides MLS addresses. Not the same as thin scheduling (Netlify cron alarm clocks).',
  },
  {
    term: 'Intelligence',
    category: 'ui-tabs',
