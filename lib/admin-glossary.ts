...
export const ADMIN_GLOSSARY: GlossaryEntry[] = [
  ...
  },
  {
    ...
    category: 'sync-admin',
    definition:
      'Neon table of public SmartMLS OpenHouse events (db/migrations/0023_open_houses.sql). /open-houses reads this table only — no RETS on the page. The hourly queue job replaces today through +90 days (so a cancelled showing disappears), stamps open_houses_synced_at, then upserts lookback in newest-first 14-day slices under an 8-minute budget so a long history pull cannot hide a finished upcoming write. Rows older than the lookback horizon are pruned. The page lists homes with a showing this Monday–Sunday week and shows past / upcoming counts from stored rows. Most open houses = top 3 historical hosts per town (ties stay). First showing = 0 past. Those two filters are mutually exclusive. Sale / rental and town pills sit in the navy hero and dock into the sticky cream bar on scroll. Open house alerts sit left of the view glyphs. MLS times stay text, not timestamps, so 11am stays 11am in Connecticut. Town sections start collapsed; drag ⋮⋮ to reorder (cookie tmre_oh_town_order).',
  },
  {
    term: 'sync_queue',
    category: 'sync-admin',
