...
export const ADMIN_GLOSSARY: GlossaryEntry[] = [
  ...
  },
  {
    ...
    category: 'sync-admin',
    definition:
      'Neon table of public SmartMLS OpenHouse events (db/migrations/0023_open_houses.sql). /open-houses reads this table only — no RETS on the page. The hourly queue job replaces today through +90 days (so a cancelled showing disappears) and stamps open_houses_synced_at. Past showings are a separate catalogue pass: npm run backfill:open-houses or scripts/backfill-open-houses.ps1 (newest-first 14-day upserts; no Railway budget). Rows older than the lookback horizon are pruned by the hourly job. The page lists homes with a showing this Monday–Sunday week and shows past / upcoming counts from stored rows. Most open houses = top 3 historical hosts per town (ties stay). First showing = 0 past. MLS times stay text, not timestamps, so 11am stays 11am in Connecticut. Filters live in a sticky bar. Town sections start collapsed; visitors reorder towns (cookie tmre_oh_town_order).',
  },
  {
    term: 'sync_queue',
    category: 'sync-admin',
