...
export const ADMIN_GLOSSARY: GlossaryEntry[] = [
  ...
  },
  {
    ...
    category: 'sync-admin',
    definition:
      'Neon table of weekly Market Pulse / Monday brief payloads. One row per Eastern send-day (slot_date). Written after a real send and by `npm run snapshot:market-pulse`. Send test does not insert a row. Stacked town points for week / month / year compares also land in stats_cache as preserved `market-pulse-week:sale:all:v1:{slotDate}` keys (snapshots freeze that Monday; a missing WoW / MoM / YoY Monday is reconstructed from listings as-of that send-day). /market-pulse subtracts a prior slot from the live stacked defaults (server-side); the page switch is Off / Week Over Week, plus Month Over Month and Year Over Year when those slots exist (stacked and unstacked). Listing town pulse and home pulse stay Off. The Monday email always draws the week-over-week change in the gold fill.',
  },
  {
