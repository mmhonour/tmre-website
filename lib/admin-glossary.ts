...
export const ADMIN_GLOSSARY: GlossaryEntry[] = [
  ...
  },
  {
    ...
    category: 'sync-admin',
    definition:
      'Railway queue job (`hero-photos`) that scavenges leftover listings missing six showcase hero index slots (`?size=full`). Oldest `list_date` first, five listings per hop, hops walk on inside the ~9-minute burst. A hop that stores nothing is skipped next burst (capped). R2 bytes with a missing index row are indexed without a Media refetch. Lowest claim rank so Incremental / stats / CAMA go first. Writes `hero_photos_status` (percent missing before, listings/photos filled, percent remaining) and `last_hero_photos`. When coverage is 100% the job still runs, only recounts, and reports idle / 100% complete on the Syncs run board. Not part of Sync all. Railway sweeps every 2m and thin cron `sync-hero-photos` every 15m; both still respect Configure. Catch-up CLI: `npm run backfill:listing-photos`. Incremental still queues ids only — it does not download photo bodies.',
  },
  {
