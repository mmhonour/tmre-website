    ...
    steps: [
      {
        ...
        detail:
            "syncCtCamaTaxHistory(). The Railway 30-min sweep and the Netlify thin */30 (sync-cama-tax) both enqueue on sync_queue at the configured monthly slot; never-finished is due immediately so a job that landed mid-month does not wait until the 1st. The runner claims one row into a forked child under Configure → Property tax history → Budget. There is no Netlify worker — a stranded row retries next tick. Per town: one request per CAMA vintage (2025/2024/2023/2022) plus the OPM mill rate table, matched to listings by vision_pid where Vision runs and by normalised street address elsewhere, then assessment ÷ 1000 × mill rate per fiscal year. Only years before the current fiscal year are written — the MLS feed owns the current year and incremental sync rewrites it every half hour, so computed rows never contest it. Norwalk is skipped: it bills through six numbered taxing districts and the published town-proper rate is not applicable per parcel. Stamps cama_tax_history_synced_at.",
        status: "scheduled",
        statusLabel: "Cron",
      },
    ],
