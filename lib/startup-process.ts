      ...
      steps: [
        {
          ...
          timing: "weekly",
          detail:
            "syncVisionAddresses(): first fillMissingVisionStreetIndex() (Streets.aspx?Letter= A–Y except X/Z into vision_streets; does not move the parcel cursor); then Streets→Parcel Field Card parse → Neon vision_addresses.field_card JSON + R2 HTML pointer; full fill then fingerprint incremental; then backfillVisionListingLinks() (lib/vision-listing-match.ts: zip strip, street type/compass, name words, exact key, trailing street type, unique MBLU; 1 PID stamps every listing at that key). Same join: npm run match:vision-listings. Netlify thin sync-vision-addresses → worker; Admin Sync now queues that worker. Skips when Pause is checked on Vision addresses (GIS).",
