...
export function describeStartupProcess(): {
    ...
    },
    {
      ...
      steps: [
        ...
        },
        {
          ...
          detail:
            "rebuildStatsCache (market stats, sales-by-month, active-by-month, vintage, price, avg-score-by-vintage) + Deal of the Day/Week caches + If estimates + location estimates (coastal / town center) + comps edges + edge scores, then stamp refresh finished.",
          status: startupFullEnabled ? "scheduled" : "skipped",
          statusLabel: startupFullEnabled ? "Chained" : "—",
        },
      ],
      ...
      steps: [
        {
          ...
          timing: "weekly",
          detail:
            "syncVisionAddresses(): for every town in VISION_GIS_TOWNS, fillMissingVisionStreetIndex() then fillMissingVisionStreetParcels() (letter pages + house lists; does not move the Field Card cursor). Adding a town is enough — if any town is still missing letters, houses, or never-ingested street-house PIDs (no vision_addresses row), the Railway 10-min sweep (and the thin cron) enqueue without waiting for the weekly slot (`visionGisNeedsCatchUp`). A stored Field Card without mailing does not keep catch-up due. Then Field Cards for those missing-owner PIDs, then the letter/street parcel walk for the current crawl town → Neon vision_addresses.field_card JSON + owner_name + R2 HTML pointer; full fill then fingerprint incremental; then backfillVisionListingLinks(). Pause still skips the job. History End is the chunk finish.",
  ...
  lanes.push({
    id: "street-listings",
    title: "Street listings (RETS)",
    subtitle:
      "Missing /streets house numbers → last MLS listing (any town SmartMLS covers)",
    steps: [
      {
        id: "street-listings-weekly",
        title: "Chunked RETS pull for unlinked street addresses",
  ...
  lanes.push({
    id: "alerts",
    title: "Listing / OH alerts",
    subtitle: "Railway mailer — two matchers, one email if both",
    steps: [
      {
        id: "alerts-railway",
