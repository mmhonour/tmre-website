    ...
    steps: [
      {
        ...
        detail:
          "syncOpenHouses(). The Railway 10-min sweep enqueues on sync_queue at the configured wall-clock slot (default every 60m); the runner claims the row into a forked child under Configure → Open houses → Budget. There is no Netlify worker — the page reads Neon only. Upcoming (today .. +90d ET) is replaced wholesale so a cancelled showing disappears; the prior year is upserted so history accumulates; a RETS fault cannot empty a window. After a successful pull, processDueSavedSearchAlerts() emails visitors who opted into open-house notify. /open-houses joins the next 7 days to listings and shows past / upcoming counts from the stored rows. Pause/Run/Reset on Admin → Syncs.",
        status: "scheduled",
        statusLabel: "Cron",
      },
    ],
