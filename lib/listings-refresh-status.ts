...
/** Active outermost lock source (cleared when the lock ends). */
const REFRESH_SOURCE_ACTIVE_KEY = 'refresh_source'
/** Source recorded when the outermost lock last finished. */
const REFRESH_SOURCE_LAST_KEY = 'last_refresh_source'

export type ListingsRefreshLockStatus = {
  inProgress: boolean
  depth: number
...
export type ListingsRefreshStatus = {
  refreshing: boolean
  lastFinishedAt: string | null
  /** Kind that produced `lastFinishedAt` (e.g. incremental, full-sync, stats-cache). */
  lastKind: string | null
  /** Kind of the in-progress lock, if any. */
...
export function readListingsRefreshLockStatus(now = Date.now()): ListingsRefreshLockStatus {
...
export function forceClearListingsRefreshLock(): ListingsRefreshLockStatus {
  const before = readListingsRefreshLockStatus()
  ...
  setSyncMeta(REFRESH_SOURCE_ACTIVE_KEY, '')
...
export function beginListingsRefresh(source: string | null = null): void {
  ...
  if (depth === 1) {
    ...
    setSyncMeta(REFRESH_SOURCE_ACTIVE_KEY, source?.trim() || 'unknown')
    void import('@/lib/listing-photos-db-persist').then(({ scheduleListingPhotosDbBlobPersist }) =>
...
export function endListingsRefresh(finishedAt?: string): void {
  ...
  if (depth === 0) {
    ...
    const source = getSyncMeta(REFRESH_SOURCE_ACTIVE_KEY)?.trim()
    finalizeActiveHistoryEntry(finished)
    setActiveRefreshLockStats(null)
    activeHistoryEntryId = null
    setSyncMeta('refresh_in_progress', '0')
    setSyncMeta('last_refresh_finished_at', finished)
    if (source) setSyncMeta(REFRESH_SOURCE_LAST_KEY, source)
    setSyncMeta(REFRESH_SOURCE_ACTIVE_KEY, '')
    void import('@/lib/listing-photos-db-persist').then(({ scheduleListingPhotosDbBlobPersist }) =>
...
function resolveLastRefreshKind(lastFinishedAt: string | null): string | null {
  if (!lastFinishedAt) return null
  const finished = getSyncMeta('last_refresh_finished_at')
  const stats = getSyncMeta('last_stats_cache')
  const full = getSyncMeta('last_full_sync')
  const source = getSyncMeta(REFRESH_SOURCE_LAST_KEY)?.trim() || null

  // Lock-end timestamp wins attribution when it is the newest stamp.
  if (sameInstant(finished, lastFinishedAt)) {
    return source || 'refresh'
  }
  if (sameInstant(stats, lastFinishedAt)) return 'stats-cache'
  if (sameInstant(full, lastFinishedAt)) return 'full-sync'
  return source
}

export function readListingsRefreshStatus(): ListingsRefreshStatus {
  healStaleRefreshLock()
  const refreshing = getSyncMeta('refresh_in_progress') === '1'
  const lastFinishedAt = latestIsoTimestamp(
    getSyncMeta('last_refresh_finished_at'),
    getSyncMeta('last_stats_cache'),
    getSyncMeta('last_full_sync'),
  )
  const activeSource = getSyncMeta(REFRESH_SOURCE_ACTIVE_KEY)?.trim() || null
  return {
    refreshing,
    lastFinishedAt,
    lastKind: resolveLastRefreshKind(lastFinishedAt),
    refreshingKind: refreshing ? activeSource || 'unknown' : null,
  }
}
