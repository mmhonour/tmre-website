...
export const ADMIN_NETLIFY_FUNCTIONS: AdminServerEntry[] = [
  ...
  },
  {
    ...
    detail:
      "Thin hero-photos trigger — enqueues hero-photos on the sync runner when Configure is due (default every 15m). Every listing missing R2 photo slots (Active first, then Closed/Expired); skip persists so leftovers still get filled. Idle when 100% complete. No Netlify worker.",
