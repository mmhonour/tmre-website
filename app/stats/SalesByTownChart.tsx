...
import { DEFAULT_COMPARE_YEARS, pillClass, sortYears, yearChartStyle } from "./stats-month-chart-utils";
import { TOWN_LIST, type StatsKind, type Town } from "./stats-towns";
import { statsByMonthTownTitle, statsClosedLabel, statsVolumeNoun } from "./stats-labels";
...
export default function SalesByTownChart({ kind }: { kind: StatsKind }) {
  const id = useId().replace(/:/g, "");
  const yearsPrefKey = `tmre_stats_sales_by_town_years:${kind}`;
  const timelinePrefKey = `tmre_stats_sales_by_town_timeline:${kind}`;
  const [byTown, setByTown] = useState<Partial<Record<Town, MonthlyCount[]>>>({});
  const [fallback, setFallback] = useState(false);
  const [loading, setLoading] = useState(true);
  const [selectedYears, setSelectedYears] = useState<Set<number>>(() =>
    new Set(readPersistedYears(yearsPrefKey, CHART_YEARS, DEFAULT_COMPARE_YEARS)),
  );
  ...
  useEffect(() => {
    setSelectedYears(new Set(readPersistedYears(yearsPrefKey, CHART_YEARS, DEFAULT_COMPARE_YEARS)));
    setTimelineMode(readPersistedTimelineMode(timelinePrefKey) ?? "calendar");
    setPrefsHydrated(true);
  }, [kind, yearsPrefKey, timelinePrefKey]);
  ...
  const monthCount = monthSlots.length;
  ...
  const tooltipFormatter = (value: unknown, name: unknown) => {
    ...
    if (calendarMultiYearMode) {
      ...
      if (parsed) {
        return [`${value} ${statsVolumeNoun(kind)}`, `${parsed.town} · ${parsed.year}`];
      }
    }
    return [`${value} ${statsVolumeNoun(kind)}`, label];
  };
    ...
    <div className="stats-chart-card rounded-2xl overflow-hidden border border-white/10 shadow-2xl shadow-navy/30">
      ...
      <div className="bg-[#0a1020] px-6 py-3 flex flex-wrap items-center justify-between gap-3">
        <p className="font-mono text-[9px] tracking-wide text-white/20">
          {statsClosedLabel(kind)} · All Towns · {viewLabel(visibleYears, timelineMode, monthCount)}
        </p>
