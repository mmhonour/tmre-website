...
| `POST /run`, `/stats`, `/scores`, `/edge-scores`, `/deal-of-the-day`, `/property-addresses`, `/vision-addresses`, `/market-digest`, `/cama-tax`, `/street-listings` | Legacy per-job aliases; they enqueue like `/enqueue` |

Runner jobs on the queue: incremental, listing-scores, edge-scores, stats-cache, deal-of-the-day, property-addresses, vision-addresses, market-digest, open-houses, **alerts**, **cama-tax**, **street-listings**. Drain claims **non-incremental jobs first** so a looping Incremental cannot starve stats / edge / CAMA / street listings. Incremental and open-houses only mark alert dirty; the **alerts** job sends listing and OH mail (one email if the visitor signed up for both). CAMA has a Netlify thin cron (`sync-cama-tax`) that enqueues, and `sync-cama-tax-worker` as stranded-row rescue when the runner is silent. Never-finished is due immediately so a mid-month deploy does not wait until the 1st. Street listings (`sync-street-listings`) is the same thin-cron shape with no worker, weekly Wed 02:00 ET plus 6h catch-up while unlinked `/streets` addresses remain.
